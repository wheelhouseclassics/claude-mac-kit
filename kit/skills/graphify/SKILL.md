...
/graphify <path> --obsidian --obsidian-dir ~/second-brain      # write vault to custom path (e.g. existing vault)
...
```bash
# Ensure the kit venv exists (Homebrew Python 3.12, falling back to python3), then install graphify into it if needed
if [ ! -x ~/.claude-kit/venv/bin/python ]; then
  mkdir -p ~/.claude-kit
  /opt/homebrew/bin/python3.12 -m venv ~/.claude-kit/venv 2>/dev/null || python3 -m venv ~/.claude-kit/venv
fi
~/.claude-kit/venv/bin/python -c "import graphify" 2>/dev/null \
  || ~/.claude-kit/venv/bin/pip install graphifyy -q 2>&1 | tail -n 3
# Write interpreter path for all subsequent steps
printf '%s' ~/.claude-kit/venv/bin/python > .graphify_python
```
...
Export it as `GRAPHIFY_WHISPER_PROMPT` in the same Bash call as the transcription command - the fence below already has the `export` line, fill in the prompt.

**Step 2 - Transcribe:**

```bash
export GRAPHIFY_WHISPER_PROMPT="WHISPER_PROMPT"   # replace WHISPER_PROMPT with the domain hint you wrote
# export GRAPHIFY_WHISPER_MODEL="base"           # uncomment and set only if --whisper-model was given
~/.claude-kit/venv/bin/python - <<'PY'
import json, os
from pathlib import Path
...
**Whisper model:** Default is `base`. If the user passed `--whisper-model <name>`, uncomment and set `export GRAPHIFY_WHISPER_MODEL="<name>"` in the command above.
...
Before dispatching, create the output directory so subagents have somewhere to write their chunk files (it is not created until Step 4 otherwise):
...
Write the result as valid JSON matching the schema below to the file graphify-out/.graphify_chunk_CHUNK_NUM.json
(exactly that path, relative to the working directory - e.g. chunk 3 writes graphify-out/.graphify_chunk_3.json).
Write it with the Write tool or with Python using encoding='utf-8' - never rely on the platform default encoding.
Then reply with the single line: wrote graphify-out/.graphify_chunk_CHUNK_NUM.json
No explanation, no markdown fences, no other output.
...
The chunk file must contain exactly this JSON (no other text):
...
- Check that `graphify-out/.graphify_chunk_N.json` exists on disk (N = chunk number, same name the subagent prompt was told to write) — this is the success signal
...
Merge the chunk files into `.graphify_semantic_new.json`:
```bash
~/.claude-kit/venv/bin/python - <<'PY'
import json
from pathlib import Path
...
print('chunk files found:', [p.name for p in chunk_files])
...
for p in chunk_files:
    ...
    hyperedges.extend(c.get('hyperedges', []))
    in_tok += c.get('input_tokens', 0)
    out_tok += c.get('output_tokens', 0)
merged = {'nodes': nodes, 'edges': edges, 'hyperedges': hyperedges, 'input_tokens': in_tok, 'output_tokens': out_tok}
Path('.graphify_semantic_new.json').write_text(json.dumps(merged, indent=2), encoding='utf-8')
print(f'{len(nodes)} nodes, {len(edges)} edges from {len(chunk_files)} chunk file(s)')
PY
```

If it prints `chunk files found: []` but uncached files existed, stop - the subagents did not write their chunk files. Do not proceed with an empty graph.
...
PY
```
Clean up temp files: `rm -f .graphify_cached.json .graphify_uncached.txt .graphify_semantic_new.json graphify-out/.graphify_chunk_*.json`
...
```bash
~/.claude-kit/venv/bin/python -m graphify.serve graphify-out/graph.json
```

This starts a stdio MCP server that exposes tools: `query_graph`, `get_node`, `get_neighbors`, `get_community`, `god_nodes`, `graph_stats`, `shortest_path`. Add to Claude Desktop or any MCP-compatible agent orchestrator so other agents can query the graph live.

To configure in Claude Desktop, add to `~/Library/Application Support/Claude/claude_desktop_config.json`. Claude Desktop does not expand `~`, so use the absolute venv path (`echo ~/.claude-kit/venv/bin/python` prints it):
```json
{
  "mcpServers": {
    "graphify": {
      "command": "/Users/YOUR_USERNAME/.claude-kit/venv/bin/python",
...
```bash
~/.claude-kit/venv/bin/python -m graphify save-result --question "QUESTION" --answer "ANSWER" --type query --nodes NODE1 NODE2
```
...
```bash
~/.claude-kit/venv/bin/python -m graphify save-result --question "Path from NODE_A to NODE_B" --answer "ANSWER" --type path_query --nodes NODE_A NODE_B
```
...
```bash
~/.claude-kit/venv/bin/python -m graphify save-result --question "Explain NODE_NAME" --answer "ANSWER" --type explain --nodes NODE_NAME
```
...
```bash
~/.claude-kit/venv/bin/python -m graphify.watch INPUT_PATH --debounce 3
```
...
```bash
~/.claude-kit/venv/bin/graphify hook install    # install
~/.claude-kit/venv/bin/graphify hook uninstall  # remove
~/.claude-kit/venv/bin/graphify hook status     # check
```
...
```bash
~/.claude-kit/venv/bin/graphify claude install
```

This writes a `## graphify` section to the local `CLAUDE.md` that instructs Claude to check the graph before answering codebase questions and rebuild it after code changes. No manual `/graphify` needed in future sessions.

```bash
~/.claude-kit/venv/bin/graphify claude uninstall  # remove the section
```
...
### Terminal output garbled or scrolling breaks after a run

If the terminal misbehaves after running graphify, this is caused by ANSI escape sequences from the `graspologic` library. Graphify v0.3.10+ suppresses this output, but if you still see the issue:

1. **Upgrade graphify**: `~/.claude-kit/venv/bin/pip install --upgrade graphifyy`
2. **Reset your terminal**: run `reset`, or close and reopen the Terminal / iTerm tab
3. **Skip graspologic**: uninstall it (`~/.claude-kit/venv/bin/pip uninstall graspologic`) and graphify will fall back to NetworkX's built-in Louvain algorithm, which produces no ANSI output. Do the same if `graspologic` fails to build on Apple Silicon - it is optional.

### `graphify` or `python` not found

Every command in this skill calls the kit venv explicitly (`~/.claude-kit/venv/bin/python`, `~/.claude-kit/venv/bin/graphify`). If the venv is missing, re-run Step 1 - it creates the venv (Homebrew Python 3.12 under `/opt/homebrew`, falling back to `python3`) and installs `graphifyy` into it.
